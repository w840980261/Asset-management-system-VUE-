<template>
  <span>
    <el-popover ref="pop" v-model="pop" :placement="placement" width="300" trigger="click">
      <div v-if="clearable" class="d2-clearfix d2-mb-10">
      ...
      <el-collapse v-if="!searchMode" v-model="collapseActive" class="d2-icon-select--group">
        <el-collapse-item v-for="(item, index) in icon" :key="index" :title="item.title" :name="index" class="d2-icon-select--class">
          <el-row class="d2-icon-select--class-row">
            <el-col v-for="(iconName, iconIndex) in item.icon" :key="iconIndex" :span="4" class="d2-icon-select--class-col" @click.native="selectIcon(iconName)">
