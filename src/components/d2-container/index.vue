<template>
  <div ref="container" class="container-component">
    ...
    <!-- [card] 卡片容器 滚动优化 -->
    <d2-container-card-bs v-if="type === 'card' && betterScroll" v-bind="$attrs">
      <slot v-if="$slots.header" slot="header" name="header"/>
      <slot/>
      <slot v-if="$slots.footer" slot="footer" name="footer"/>
    ...
    <!-- [ghost] 隐形容器 滚动优化 -->
    <d2-container-ghost-bs v-if="type === 'ghost' && betterScroll" v-bind="$attrs">
      <slot v-if="$slots.header" slot="header" name="header"/>
      <slot/>
      <slot v-if="$slots.footer" slot="footer" name="footer"/>
    ...
    <d2-container-full-bs v-if="type === 'full' && betterScroll" v-bind="$attrs">
      <slot v-if="$slots.header" slot="header" name="header"/>
      <slot/>
      <slot v-if="$slots.footer" slot="footer" name="footer"/>
