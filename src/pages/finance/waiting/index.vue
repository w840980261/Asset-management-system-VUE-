<template>
  <d2-container>
    ...
    >
      ...
      <el-table-column prop="inOperator" label="入库操作员" align="center"/>
...
export default {
  ...
  methods: {
    ...
    getTableData () {
      ...
      billActionService.BillOutList(param).then(data => {
    ...
    entry (id) {
      this.$confirm('确认入账？', '确认信息', {
        distinguishCancelAndClose: true,
        confirmButtonText: '确认',
        cancelButtonText: '取消'
      }).then(() => {
        const data = {
          'entryOperator': 'yangzaiqiong',
          'oderNoList': [id]
        }
        billActionService.InBill(data).then(() => {
          if (!data.hasOwnProperty('code')) {
            this.getTableData()
          }
        })
